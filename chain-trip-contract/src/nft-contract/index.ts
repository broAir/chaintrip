...
import { NearContract, NearBindgen, near, call, view, assert, LookupMap, UnorderedMap, Vector, UnorderedSet } from 'near-sdk-js'
...
export class Contract extends NearContract {
    ...
    */
    constructor({
        ...
        metadata = {
            spec: "nft-1.0.0",
            name: "dAGI Testnet Corgi trip",
            symbol: "TCORGI",
            promptHashed: "MjU2eDI1NiBhIGNvcmdpIGRvZyBwc3ljaGVkZWxpYyBzdHlsZSA=" // TODO; this will be instantited by a Contract factory
        }
    }) {
        super()
        ...
        this.promptHashed = metadata.promptHashed;
    }
    ...
    */
    @view
    //get the prompt for the contract
    nft_prompt() {
        return this.promptHashed;
    }

    @call
    //set the prompt for the contract
    nft_set_prompt({ prompt_hashed }) {
        assert(near.currentAccountId() === near.predecessorAccountId(), "Only the contract itself can call this method");
        assert(prompt_hashed.length > 0, "Prompt cannot be empty")
        assert(this.promptHashed !== prompt_hashed, "Prompt already set");

        this.promptHashed = prompt_hashed;
    }
    ...
    */
    @call
    nft_mint({ token_id, metadata, receiver_id, perpetual_royalties, new_prompt_hashed }) {
        assert(near.currentAccountId() === this.owner_id, "Only the owner can mint new tokens");

        this.promptHashed = new_prompt_hashed;
